...
|---|---|---|
| Pre-compact gate hook (`memento/tools/pre-compact-gate.sh`) | — (wired at Slice 1: compaction loss is the framework's core failure mode, pre-earned by the lineage) | **WIRED 2026-07-20, teeth UNVERIFIED** — blocking convention (JSON block decision on stdout) imported from the lineage's live-proven hook per founding review finding 2; untested in THIS estate; witness at first live `/compact` and record the verdict on this line |
