...
| Confidentiality sweep + pre-push hook (`memento/tools/confidentiality-sweep.sh`, `.githooks/pre-push`; token list outside the repo at `~/.memento/banned-tokens.txt`) | **Fired 2026-07-21:** the external re-review caught banned literals published inside the memo documenting their banning (the manual sweep had run before the final writes) | **WIRED 2026-07-21; bench-proven on all three arms** (clean run 96 tracked files; block arm caught a REAL residual leak at bench — the memo's token-example parenthetical; missing-list arm blocks). Hook activated via `core.hooksPath .githooks` (per-clone). **Pre-push live arm UNVERIFIED** until the next `git push` runs it. Falsifier: a push that carries a listed token the hook allows, or a hook bypass nobody notices (witness: the sweep's clean line in the push transcript) |
